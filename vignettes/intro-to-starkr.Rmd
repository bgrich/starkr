...
```{r stark_map calc}
stark_map <- starkr::stark_eigen(mat, zero_energy_mat, 0, 4, 0.5)
stark_map[1:4, 1:4]
```
...
energy matrix as starting arguments. The field minimum, field maximum, and 
step size must be the same as used in `stark_eigen()`. `tidy_stark_energy()` 
...
```{r tidy df}
tidy_df <- starkr::tidy_stark_energy(zero_df, stark_map, 0, 4, 0.5)
tidy_df
```
...
stark_radial_mat_elem(mat, "5,1,1.5,1.5", "15,1,1.5,1.5", field = 10, n_min = 15, n_max = 15, n_add_min = 0, n_add_max = 0)
```
